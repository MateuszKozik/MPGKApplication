...
public class ActivityGroupService {
    ...
    public ActivityGroup get(Long groupId) {
        return activityGroupRepository.findById(groupId).orElseThrow(() -> new ActivityGroupNotFoundException(groupId));
    ...
    public ActivityGroup update(Long groupId, ActivityGroup activityGroup) {
        return activityGroupRepository.findById(groupId).map(element -> {
